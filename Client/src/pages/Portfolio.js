import React from 'react';
import '../styles/style.css';

function Portfolio() {
    return (

            

        <div className="section no-pad-bot display:flex" id="index-banner">
                <div id="faq" className="faqContainer">
                <div className="container">
                <br /><br />
                
                
                
        <div>
          <h2>Work</h2>
        </div>

      <div className="content2">
        <div className="top-project">
          <div className="Surf-Report">
            <p>
              <a href="#Surf-Report">Surf Report</a>
              <br />
              <font size="-0.5">MERN-Stack Web Application</font>
            </p>
          </div>
        </div>

        <div className="smaller-projects">
          <div className="LED-Wall">
            <p>
              <a href="https://mzernick.github.io/Drink-of-the-Day/" calss="#LED-Wall">
                Today's Drink
              </a>
              <br />
              <font size="-0.5">Node/IoT</font>
            </p>
          </div>

          <div className="Calculator">
            <p>
              <a href="#Calculator">Calculator</a>
              <br />
              <font size="-0.5">React/JavaScript/CSS</font>
            </p>
          </div>

          <div className="Pastel-Puzzles">
            <p>
              <a href="#Pastel-Puzzles">Pastel Puzzles</a>
              <br />
              <font size="-0.5">MERN stack</font>
            </p>
          </div>

          <div className="Run-Buddy">
            <p>
              <a href="#Run-Buddy">Run Buddy</a>
              <br />
              <font size="-0.5">HTML/CSS</font>
            </p>
          </div>
        </div>
      </div>




</div>
            
                <br /><br />


            
            </div>
        </div>

        

        



    );
}

export default Portfolio;